...
export default class DemoBlock extends Vue {
    ...
    @Watch('isExpanded')
    isExpandedWatch (val: boolean) {
        (this.codeArea as any).style.height = val
            ? `${this.codeAreaHeight + 1}px`
            : "0";
        if (!val) {
            this.fixedControl = false;
            (this.$refs.control as any).style.left = "0";
    ...
    scrollHandler() {
        const { top, bottom, left } = (this.$refs.meta as any).getBoundingClientRect();
        this.fixedControl =
            bottom > document.documentElement.clientHeight &&
            top + 44 <= document.documentElement.clientHeight;
        (this.$refs.control as any).style.left = this.fixedControl ? `${left}px` : "0";
    }
    ...
    mounted() {
        this.$nextTick(() => {
            ...
            if (this.$el.getElementsByClassName("description").length === 0) {
                (highlight as any).style.width = "100%";
                (highlight as any).borderRight = "none";
            }
        });
    }
